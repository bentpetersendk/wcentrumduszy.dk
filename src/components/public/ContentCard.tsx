...
export function ContentCard({ item, href, cta = "Read more" }: { item: CmsContent; href: string; cta?: string }) {
  return (
    <article className="grid gap-5 rounded-md border border-border bg-surface p-5 sm:grid-cols-[0.42fr_1fr]">
      ...
      <div className="flex flex-col">
        ...
        <div className="mt-5 flex flex-wrap gap-x-4 gap-y-2 text-caption text-text-muted">
          {typeof item.metadata.date === "string" ? <span>{item.metadata.date}</span> : null}
          {typeof item.metadata.duration === "string" ? <span>{item.metadata.duration}</span> : null}
          {typeof item.metadata.category === "string" ? <span>{item.metadata.category}</span> : null}
